...
static void init_shaders(PCUBE_STATE_T state)
{
    ...
    FILE* vertex_shader_file = fopen("shader.vert", "r");
    fseek(vertex_shader_file, 0, SEEK_END);
    size_t vertex_shader_size = ftell(vertex_shader_file);
    fseek(vertex_shader_file, 0, SEEK_SET);
    GLchar* vertex_shader_source = (GLchar*)calloc(vertex_shader_size, sizeof(GLchar));
    fread(vertex_shader_source, sizeof(GLchar), vertex_shader_size, vertex_shader_file);
    fclose(vertex_shader_file);
    state->vertex_shader = glCreateShader(GL_VERTEX_SHADER);
    glShaderSource(state->vertex_shader, 1, (const GLchar**)&vertex_shader_source, 0);
    glCompileShader(state->vertex_shader);
    check(); if (state->verbose) { show_log(state->vertex_shader); }
    free(vertex_shader_source);

    if (state->verbose) { printf("[INFO]: %s\n", "Compiling Fragment Shader..."); }
    FILE* fragment_shader_file = fopen("shader.frag", "r");
    ...
    GLchar* fragment_shader_source = (GLchar*)calloc(fragment_shader_size, sizeof(GLchar));
    fread(fragment_shader_source, sizeof(GLchar), fragment_shader_size, fragment_shader_file);
    fclose(fragment_shader_file);
    state->fragment_shader = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(state->fragment_shader, 1, (const GLchar**)&fragment_shader_source, 0);
